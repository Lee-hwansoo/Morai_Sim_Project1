...
class LaneDetectionROS:
    def __init__(self):
        ...
        self.image_parser = IMGParser()
        self.lane_detector = HoughLaneDetector()
        self.bridge = CvBridge()

    def callback(self, msg):
        try:
            self.image_parser.img_bgr = self.convert_msg_to_cv2(msg)
            self.run_detection(self.image_parser.img_bgr)
    ...
    def run_detection(self, img_bgr):
        img_warp = self.image_parser.warp_image(img_bgr)
        img_lane_edge = self.image_parser.preprocess(img_warp)
        lines = self.lane_detector.detect_lanes(img_lane_edge)
        left_lines, right_lines = self.lane_detector.separate_lanes(lines, img_warp.shape[1])

        if len(left_lines) != 0 and len(right_lines) != 0:
            left_average_lane = self.lane_detector.extract_weighted_average_lane(left_lines)
            right_average_lane = self.lane_detector.extract_weighted_average_lane(right_lines)
            if left_average_lane is not None and right_average_lane is not None:
                img_warp = self.image_parser.draw_lane_img(np.copy(img_warp), left_average_lane, right_average_lane, color=(0, 0, 255))

        concatenated_image = np.concatenate((img_bgr, img_warp), axis=1)
